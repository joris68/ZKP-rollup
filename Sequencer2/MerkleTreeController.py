...
class MerkleTreeController:
    ...
    async def update_sender_leaf_on_disk_return_bytes(self, badge_id : str, transaction : Transaction) -> bytes:
        async with await self.mongo_client.start_session(causal_consistency=True) as session:
                try:
                    logger.info(f"updating leaf data for change log")
                    ...
                    if update_index is not None:
                        ...
                        return self.leaf_to_bytes(new_balance, new_nonce, sender=transaction.sender)
                    else:
                            balance_before = None
                            nonce_before = None
    ...
    async def update_receiver_bytes_on_disk_return_bytes()


    async def leaf_to_bytes(self, balance : int, nonce : int , sender : str ) -> bytes:

        nonce_bytes = int(nonce).to_bytes(8, 'little') 
        balance_bytes = int(balance).to_bytes(8, 'little')
        hashes_pub = hashlib.sha256(bytes.fromhex(sender)).digest()
