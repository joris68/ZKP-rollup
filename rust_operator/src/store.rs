...
use smt::{
    address_to_tree_key, h256_to_hex, hex_to_address, new_smt, root, update, AccountData, SMT,
};
...
fn initialize_merkle_tree(account_information: &[AccountInfo], tree: &mut SMT) -> String {
    for (i, acc) in account_information.iter().enumerate() {
        ...
        println!(
            "DEBUG: Inserting account {} ({}) with balance={}, nonce={}",
...
fn apply_transaction(
    ...
    println!(
        ...
        from_idx, account_information[from_idx].balance, account_information[from_idx].nonce
    );
    println!(
        "  Account {}: balance={}, nonce={}",
        to_idx, account_information[to_idx].balance, account_information[to_idx].nonce
    );
    ...
    let from_address_bytes =
        hex_to_address(&account_information[from_idx].address).expect("Invalid address");
    ...
    let to_address_bytes =
        hex_to_address(&account_information[to_idx].address).expect("Invalid address");
...
//     println!(
//         "DEBUG: Starting batch creation with {} accounts, {} transactions",
